...
enum TicketNewError {
    InvalidTitle { err_msg: String },
    InvalidDescription { err_msg: String },
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::InvalidTitle { err_msg }) => {
            // Handle based on the specific error message
            panic!("{:?}", TicketNewError::InvalidTitle { err_msg })
        }
        Err(TicketNewError::InvalidDescription { err_msg: _ }) => {
    ...
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::InvalidTitle {
                err_msg:"Title cannot be empty".to_string()
            });
        }
        if title.len() > 50 {
            return Err(TicketNewError::InvalidTitle {
                err_msg:"Title cannot be longer than 50 bytes".to_string()
            });
        }
        if description.is_empty() {
            return Err(TicketNewError::InvalidDescription {
                err_msg:"Description cannot be empty".to_string()
            });
        }
        if description.len() > 500 {
            return Err(TicketNewError::InvalidDescription {
                err_msg:"Description cannot be longer than 500 bytes".to_string()
            });
        }
